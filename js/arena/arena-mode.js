...
import {clampToStage,getArenaStage,listArenaStages,outsideStageProjectileBounds,stageProfile,stageWallAvoidance} from './arena-stages.js?v=29a301-stability-cleanup-20260731';
import {drawArenaStage} from './arena-stage-renderer.js';
import {WebGLArenaRenderer} from './webgl-renderer.js';
import {ArenaControlManager} from './arena-controls.js?v=29a301-stability-cleanup-20260731';
import {ABILITY_CATEGORY,ARENA_NORMAL_PROFILES,SPECIAL_CATEGORIES,abilityCategory,abilityTiming,arenaAttackFor} from './arena-combat-data.js?v=29a301-stability-cleanup-20260731';
import {ROSTER} from '../roster.js?v=29a301-stability-cleanup-20260731';
import {COMBAT_RULES,difficultyProfile,decayHabit} from '../combat-core.js?v=29a301-stability-cleanup-20260731';
import {resolveHubWorldCollision} from '../story/hub-collision.js?v=29a301-stability-cleanup-20260731';
import {loadQolSettings} from '../qol-settings.js?v=29a301-stability-cleanup-20260731';
import {createTrainingTrialState,recordTrainingTrialEvent,resetTrainingTrial,trainingTrialView} from '../training-trials.js?v=29a301-stability-cleanup-20260731';
import {PURSUIT_TUNING,canGroundBounce,canWallSplat,dashIdentityFor,pursuitDurationFor,pursuitPromptText,pursuitTechAvailable,pursuitWindowFor} from './pursuit-combat.js?v=29a301-stability-cleanup-20260731';
import {FOCUS_RECOVERY_RULES,channelFocusRecovery,endFocusRecovery,focusRecoveryAvailability,interruptFocusRecovery,recoverableHealthView,registerRecoverableDamage,resetFocusRecovery,tickFocusRecovery} from '../focus-recovery.js?v=29a301-stability-cleanup-20260731';
...
<div class="arenaStageSelect hidden" data-arena-stage-select role="dialog" aria-modal="true" aria-label="Arena select"><div class="arenaStageCard">
  <header><small>PROTOTYPE 2.9A.30.1</small><h2>SELECT ARENA</h2><p>Choose a stage by its playstyle. Size, boundaries, pursuit lanes, wall pressure, and ring-out rules shape each match.</p></header>
...
<div class="badge"><strong>PROTOTYPE 2.9A.30.1 • STABILITY & INTERFACE CLEANUP</strong><span data-stage-name>TANGAI DOJO</span><br>REUSABLE ARENA PIPELINE • DATA-DRIVEN STAGE</div><div class="banner hidden" data-banner></div>
...
<div class="arenaControlSettings hidden" data-arena-control-settings role="dialog" aria-modal="true" aria-label="Arena control settings"><div class="arenaControlCard">
  <header><div><small>PROTOTYPE 2.9A.30.1</small><h2>CONTROL LAYOUTS</h2><p>Desktop and mobile settings save separately from combat balance.</p></div><button data-arena-controls-close aria-label="Close">×</button></header>
...
class ArenaBattle{
  ...
  applyDamage(attacker,target,damage,{guardDamage=Math.max(1,damage*.2),knockback=28,color=attacker?.accent||'#ffffff',hitstop=5,stun=.24,launch=0,spike=0,knockdown=false,kind='light'}={}){
    ...
    target.hp=Math.max(0,target.hp-finalDamage);registerRecoverableDamage(target,finalDamage,{strong:['heavy','airHeavy','launcher','pursuitHeavy','ultimate'].includes(kind)});target.stun=Math.max(target.stun,stun);target.inv=.045;target.flash=.12;target.guardDelay=1.1;target.kvx=aim.x*knockback;target.kvz=aim.z*knockback;
  ...
  finishRound(){if(this.phase!=='play')return;const[a,b]=this.fighters,winner=b.hp>a.hp?1:a.hp>b.hp?0:(Math.random()<.5?0:1);this.lastLoser=1-winner;this.scores[winner]++;this.phase='over';this.phaseTime=.72;for(const fighter of this.fighters){this.clearPursuitState(fighter);endFocusRecovery(fighter,{applyRecovery:false})}this.root.classList.remove('pursuit-active');this.banner(`K.O. • ${this.scores[0]}–${this.scores[1]}`);this.audio.play('ko');this.hud()}
