export class LoadingManager{
  ...
  set(value,category,message){if(this.progress)this.progress.style.width=`${Math.max(0,Math.min(100,value))}%`;if(category)this.category.textContent=category;if(message)this.message.textContent=message}
  finish(){this.set(100);this.root?.classList.add('hidden')}
  fail(message='An asset could not load.'){this.message.textContent=message;this.actions?.classList.remove('hidden')}
}
