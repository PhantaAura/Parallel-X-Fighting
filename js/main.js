...
function update(dt){if(state!=='playing'||paused)return;pollGamepads();if(hitstop>0){hitstop--;return}time=Math.max(0,time-dt);F.forEach(f=>f.update());P.forEach(p=>p.update());particles.forEach(p=>p.update());P=P.filter(p=>!p.dead);particles=particles.filter(p=>p.l>0);if(F[0].hp<=0||F[1].hp<=0||time<=0){let w=F[0].hp===F[1].hp?F[Math.random()<.5?0:1]:(F[0].hp>F[1].hp?F[0]:F[1]);over(w)}for(let k in pressed)delete pressed[k]}
...
if(F.length){
  ...
  if(blinded){
    ...
    ctx.font='bold 16px Segoe UI';ctx.fillStyle='#cfd6ff';ctx.fillText('VISION LOST • AUTO-DODGE ACTIVE',W/2,H/2+24);
    ctx.restore();
  }
F.forEach((f,i)=>comboHud[i].innerHTML=f.combo.hits>1?`${f.combo.hits} HIT COMBO<small>${f.combo.damage.toFixed(1)} DAMAGE • ${Math.round(f.combo.scale*100)}% SCALE</small>`:'');U.p1h.style.width=F[0].hp+'%';U.p2h.style.width=F[1].hp+'%';U.p1e.style.width=F[0].en+'%';U.p2e.style.width=F[1].en+'%';U.timer.textContent=Math.ceil(time)}}
