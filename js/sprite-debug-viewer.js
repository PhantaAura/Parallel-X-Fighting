...
export class SpriteDebugViewer{
  constructor(visuals){this.visuals=visuals;this.root=null;this.animator=null;this.frameHandle=0;this.last=0;this.flip=false;this.mobileScale=1;this.appearance='down'}
  mount(){
    if(this.root)return;
    ...
    root.innerHTML=`<header><strong>RRVVFO SPRITE VIEWER</strong><button data-debug="close">×</button></header>
      ...
      <div class="spriteDebugControls">
        ...
        <label>Background <select data-debug="background"><option value="black">Black</option><option value="white">White</option><option value="red">Red</option><option value="blue">Blue</option><option value="checker" selected>Transparent checkerboard</option></select></label>
  ...
  bind(){
    ...
    get('appearance').onchange=event=>{this.appearance=event.target.value==='up'?'up':'down';this.animator?.setAppearance(this.appearance)};
  ...
  refresh(){
    ...
    if(!this.animator)this.animator=new SpriteAnimator(this.visuals.atlas,{appearance:this.appearance});
  ...
  drawBackground(ctx,canvas,background){
    const colors={black:'#000',white:'#fff',red:'#b71924',blue:'#1949b7'};
    if(background!=='checker'){ctx.fillStyle=colors[background]||'#000';ctx.fillRect(0,0,canvas.width,canvas.height);return}
    const size=16;for(let y=0;y<canvas.height;y+=size)for(let x=0;x<canvas.width;x+=size){ctx.fillStyle=(x/size+y/size)%2?'#dadada':'#777';ctx.fillRect(x,y,size,size)}
  }
  loop(now){
    ...
    if(this.animator&&this.visuals.renderer){
      this.animator.setAppearance(this.appearance);this.animator.update(Math.min(50,now-(this.last||now)));this.last=now;
