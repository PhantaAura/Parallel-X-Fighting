...
export class PauseMenu{
  ...
  show({training=false,touch=false,owner=1,local=false}={}){if(!this.root)return;this.owner.textContent=local?`Paused by Player ${owner}`:'Combat paused';this.actions.innerHTML=PAUSE_ACTIONS.filter(action=>(!action.trainingOnly||training)&&(!action.touchOnly||touch)).map((action,index)=>`<button class="${index?'secondary':'primary'}" data-pause-action="${action.id}">${action.label}</button>`).join('');this.root.classList.remove('hidden');this.actions.querySelector('button')?.focus()}
