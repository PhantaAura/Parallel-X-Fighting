"use strict";
import {FIGHTER_META,FIGHTER_STATUS,PLAYABLE_ROSTER_IDS,ROSTER,ROSTER_IDS,isMirrorMatch} from './roster.js?v=29a4071-chapter3-sabotage-investigation-20260802';
import {STAGES,drawStage} from './stages.js';
import {CUSTOM_CONTROLLER_ACTIONS} from './input.js?v=29a4071-chapter3-sabotage-investigation-20260802';
import {sharedInput as input} from './input-runtime.js?v=29a4071-chapter3-sabotage-investigation-20260802';
import {decideCPU} from './ai.js';
import {TimerRegistry,clamp,resetCombo} from './combat.js';
import {EffectSystem} from './effects.js';
import {Fighter} from './fighter.js?v=29a4071-chapter3-sabotage-investigation-20260802';
...
import {TouchControls} from './touch-controls.js?v=29a4071-chapter3-sabotage-investigation-20260802';
import {TouchSettingsPanel,createDefaultTouchSettings} from './touch-layout-editor.js?v=29a4071-chapter3-sabotage-investigation-20260802';
import {FighterVisuals,availableRrvvfoAppearances,isDeveloperSpriteBuild,loadRrvvfoVisualSettings,normalizeRrvvfoAppearance,shouldShowRrvvfoLoadFailure} from './fighter-visuals.js?v=29a4071-chapter3-sabotage-investigation-20260802';
import {SpriteDebugViewer} from './sprite-debug-viewer.js';
import {ControllerManager} from './controller-manager.js?v=29a4071-chapter3-sabotage-investigation-20260802';
import {BUILD_VERSION} from './build-info.js?v=29a4071-chapter3-sabotage-investigation-20260802';
import {initializeStoryPolish} from './story/story-polish.js?v=29a4071-chapter3-sabotage-investigation-20260802';
import {initializeStoryCharm} from './story/story-charm.js?v=29a4071-chapter3-sabotage-investigation-20260802';
import {initializeWorldDelight} from './story/world-delight.js?v=29a4071-chapter3-sabotage-investigation-20260802';
import {initializeMobileStoryUi} from './story/mobile-story-ui.js?v=29a4071-chapter3-sabotage-investigation-20260802';
import {ConfirmationDialog} from './confirmation-dialog.js';
import {MainMenu} from './main-menu.js?v=29a4071-chapter3-sabotage-investigation-20260802';
import {MatchStatistics} from './match-statistics.js';
import {PauseMenu,simulationCanAdvance} from './pause-menu.js?v=29a4071-chapter3-sabotage-investigation-20260802';
import {ResultsScreen} from './results-screen.js';
import {loadQolSettings,saveQolSettings} from './qol-settings.js?v=29a4071-chapter3-sabotage-investigation-20260802';
import {NotificationSystem} from './notification-system.js';
import {adaptiveMoveList,renderAdaptiveMoveList} from './move-list.js';
import {SettingsPanel} from './settings-panel.js?v=29a4071-chapter3-sabotage-investigation-20260802';
import {importSaveText,resetSaveGroup,stringifySave} from './save-manager.js?v=29a4071-chapter3-sabotage-investigation-20260802';
...
import {cooldownText,fighterHudModel} from './hud-model.js?v=29a4071-chapter3-sabotage-investigation-20260802';
import {AbilityHotbar} from './ability-hotbar.js?v=29a4071-chapter3-sabotage-investigation-20260802';
import {loadAbilityHotbarSettings,saveAbilityHotbarSettings} from './ability-hotbar-data.js?v=29a4071-chapter3-sabotage-investigation-20260802';
import {ResponsiveGameLayout} from './responsive-game-layout.js?v=29a4071-chapter3-sabotage-investigation-20260802';
import {OrientationManager,loadMobilePresentationSettings,saveMobilePresentationSettings} from './orientation-manager.js?v=29a4071-chapter3-sabotage-investigation-20260802';
import {FullscreenManager} from './fullscreen-manager.js?v=29a4071-chapter3-sabotage-investigation-20260802';
...
if(mobilePlatform.info.touch&&input.lastInputDevice[0]==='keyboard')input.lastInputDevice[0]='touch';
...
function handleSettingsAction(action){
  ...
  if(action==='resetHubCamera'){import('./story/hub-camera.js?v=29a4071-chapter3-sabotage-investigation-20260802').then(({resetAllHubCameras})=>{resetAllHubCameras();notifications.push('HUB CAMERA RESET',{important:true,key:'camera-reset'})});return}
...
function showExtra(section){const content=$('extrasContent');if(section==='manual'){import('./story/combat-manual.js?v=29a4071-chapter3-sabotage-investigation-20260802').then(({openCombatManual})=>{openCombatManual({grantPublic:true,onClose:()=>{$('extrasPanel').classList.remove('hidden')}});$('extrasPanel').classList.add('hidden')});return}if(section==='moves'){openAdaptiveMoveList(p1id,1);return}if(section==='profiles')content.innerHTML=Object.entries(ROSTER).map(([id,fighter])=>`<p><strong>${fighter.n}</strong> — ${FIGHTER_META[id].style} <small>• ${FIGHTER_STATUS[id]?.label||'PROTOTYPE'}</small></p>`).join('');else if(section==='stages')content.innerHTML=Object.entries(STAGES).map(([id,data])=>`<p><strong>${data.n}</strong> — ${STAGE_DETAILS[id].description}</p>`).join('');else if(section==='controls'){openControlsPanel();return}else if(section==='mastery'){import('./mastery-records.js?v=29a4071-chapter3-sabotage-investigation-20260802').then(({loadMasteryRecords,renderMasteryRecords})=>{content.innerHTML=renderMasteryRecords(loadMasteryRecords(resilientStorage))});return}else if(section==='builds'||section==='adventure'){import('./story/core-fun.js?v=29a4071-chapter3-sabotage-investigation-20260802').then(({renderCoreFunExtras,bindRrvvfoBuildLab})=>{content.innerHTML=renderCoreFunExtras({storage:resilientStorage});const lab=content.querySelector('[data-extras-build-lab]');if(lab)bindRrvvfoBuildLab(lab,{storage:resilientStorage,onChange:()=>{content.innerHTML=renderCoreFunExtras({storage:resilientStorage});const next=content.querySelector('[data-extras-build-lab]');if(next)bindRrvvfoBuildLab(next,{storage:resilientStorage})}})});return}else if(section==='credits')content.innerHTML='<strong>Parallels X: Clash of Souls</strong><p>Original characters and game direction by the Parallels X creator. Browser prototype engineering developed collaboratively with Codex.</p>';else content.innerHTML=`<strong>${BUILD_VERSION}</strong><p>Living Tournament Hub • Chapters 1–4 • Story RPG stats • Side quests • Tournament intermissions • Kinetic combat</p>`}
...
function buildRoster(){
  ...
  for(const id of ROSTER_IDS){
    ...
    const portrait=playable
      ?`<div class="portrait fighterPortrait fighterPortrait-${id}" aria-hidden="true"><span></span></div>`
      :`<div class="portrait placeholderPortrait" aria-hidden="true"><div class="head" style="background:${c.h}"></div><div class="body" style="background:${c.c};box-shadow:0 0 12px ${c.a}66"></div></div>`;
    button.innerHTML=`<span class="fighterStatus">${playable?status.label:'COMING SOON'}</span>${portrait}<b>${c.n.toUpperCase()}</b><div class="origin">${c.o}</div><div class="style">${playable?meta.style:'MOVESET AND VISUALS IN DEVELOPMENT'}</div><div class="rating">${playable?'◆'.repeat(meta.difficulty)+'◇'.repeat(5-meta.difficulty):'LOCKED'}</div>`;
...
async function startGameInternal(){
  ...
  loadingManager.start('MATCH INITIALIZATION','Reading fighter manifests…',{
    fighterId:['rrvvfo','revvfo','wade','bark','sage'].includes(p1id)?p1id:'rrvvfo',
  ...
  try{
    ...
    loadingManager.identity({fighterId:['rrvvfo','revvfo','wade','bark','sage'].includes(p1id)?p1id:'rrvvfo',fighterName:ROSTER[p1id]?.n||p1id,opponentName:ROSTER[p2id]?.n||p2id,stageName:STAGES[stage]?.n||stage,accent:ROSTER[p1id]?.a||'#55d9ff'});
    ...
    if(['cpu','local'].includes(requestedMode)){
      ...
      const configured=startConfiguredArenaBattle({
        mode:requestedMode,
        fighters:[p1id,p2id],
        appearances:[selectedAppearance(1),selectedAppearance(2)],
        stageId:requestedStage,
        difficulty:U.diff.value,
        koTarget:requestedMode==='cpu'?Math.max(1,Number(U.rounds.value)||1):SONIC_KO_TARGET
      });
      if(!configured)throw new Error('The shared 3D battle engine could not start.');
      return;
    }
...
const activateStartFromPointer=event=>{
  ...
  if(event?.pointerType&&event.pointerType!=='mouse')input.lastInputDevice[0]='touch';
