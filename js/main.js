...
const U={menu:$('menuScreen'),game:$('gameScreen'),mode:$('mode'),diff:$('difficulty'),stage:$('stage'),rt:$('roundTime'),rounds:$('rounds'),cine:$('cinematics'),reduced:$('reducedShake'),roster:$('roster'),slot1:$('slot1'),slot2:$('slot2'),n1:$('name1'),n2:$('name2'),m1:$('moves1'),m2:$('moves2'),s2l:$('slot2label'),notice:$('notice'),p1n:$('p1name'),p2n:$('p2name'),p1h:$('p1hp'),p2h:$('p2hp'),p1e:$('p1en'),p2e:$('p2en'),p1g:$('p1guard'),p2g:$('p2guard'),p1d:$('p1defense'),p2d:$('p2defense'),timer:$('timer'),rl:$('roundLabel'),msg:$('msg'),mt:$('msgTitle'),mx:$('msgText'),mb:$('msgButton'),pause:$('pause')};
...
let selectSlot=1,p1id='rrvvfo',p2id='revvfo',mode='story',difficulty='normal',stage='dojo',limit=90,roundsToWin=2,currentRound=1,wins1=0,wins2=0,state='menu',paused=false,story=0,time=90,last=0,acc=0,roundIntro=0;
...
function sound(cue=220,duration=.05,type='square',volume=.03){if(typeof cue==='string')audio.play(cue);else audio.tone(cue,duration,type,volume)}
...
function over(winner){
  state='over';clearTransient();winner.victory=1;roundBanner.textContent='K.O.';roundBanner.classList.remove('hidden');sound('ko');const p1win=winner===world.fighters[0];if(p1win)wins1++;else wins2++;const matchWon=wins1>=roundsToWin||wins2>=roundsToWin;
...
function update(dt){
  ...
  if(roundIntro>0){roundIntro--;if(roundIntro===48){roundBanner.textContent='FIGHT!';sound('fight')}if(!roundIntro){roundBanner.classList.add('hidden');input.clear()}return}
  updateCamera(world);
...
document.querySelectorAll('[data-t]').forEach(button=>{const map={left:['KeyA'],right:['KeyD'],jump:['KeyW'],block:['KeyS'],attack:['KeyF'],heavy:['KeyR'],throw:['KeyF','KeyR'],breaker:['KeyS','KeyG'],special:['KeyG'],ult:['KeyH'],lens:['KeyZ']},keys=map[button.dataset.t],down=event=>{event.preventDefault();keys.forEach(key=>input.setTouch(key,true))},up=event=>{event.preventDefault();keys.forEach(key=>input.setTouch(key,false))};button.onpointerdown=down;button.onpointerup=up;button.onpointercancel=up;button.onpointerleave=up});
