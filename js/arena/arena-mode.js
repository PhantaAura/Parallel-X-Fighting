...
import {clampToStage,getArenaStage,listArenaStages,outsideStageProjectileBounds,stageProfile,stageWallAvoidance} from './arena-stages.js?v=29a361-chapter4-replay-overlay-fix-20260801';
import {drawStagePersonality,projectileHitsStageGeometry,resolveStageGeometry,stagePersonalityFor} from './stage-personality.js?v=29a361-chapter4-replay-overlay-fix-20260801';
import {drawArenaStage} from './arena-stage-renderer.js';
import {WebGLArenaRenderer} from './webgl-renderer.js';
import {ArenaControlManager} from './arena-controls.js?v=29a361-chapter4-replay-overlay-fix-20260801';
import {ABILITY_CATEGORY,ARENA_NORMAL_PROFILES,SPECIAL_CATEGORIES,abilityCategory,abilityTiming,arenaAttackFor,fighterFeelFor} from './arena-combat-data.js?v=29a361-chapter4-replay-overlay-fix-20260801';
import {finalKoImpactFor,signatureMeleeKind,signatureProfileFor} from './signature-combat.js?v=29a361-chapter4-replay-overlay-fix-20260801';
import {PLAYABLE_ROSTER_IDS,ROSTER} from '../roster.js?v=29a361-chapter4-replay-overlay-fix-20260801';
import {COMBAT_RULES,difficultyProfile,decayHabit} from '../combat-core.js?v=29a361-chapter4-replay-overlay-fix-20260801';
import {resolveHubWorldCollision} from '../story/hub-collision.js?v=29a361-chapter4-replay-overlay-fix-20260801';
import {loadQolSettings} from '../qol-settings.js?v=29a361-chapter4-replay-overlay-fix-20260801';
import {createTrainingTrialState,recordTrainingTrialEvent,resetTrainingTrial,trainingTrialView} from '../training-trials.js?v=29a361-chapter4-replay-overlay-fix-20260801';
import {PURSUIT_TUNING,canGroundBounce,canWallSplat,dashIdentityFor,pursuitDurationFor,pursuitPromptText,pursuitTechAvailable,pursuitWindowFor} from './pursuit-combat.js?v=29a361-chapter4-replay-overlay-fix-20260801';
import {FOCUS_RECOVERY_RULES,channelFocusRecovery,endFocusRecovery,focusRecoveryAvailability,interruptFocusRecovery,recoverableHealthView,registerRecoverableDamage,resetFocusRecovery,tickFocusRecovery} from '../focus-recovery.js?v=29a361-chapter4-replay-overlay-fix-20260801';
import {BUILD_VERSION} from '../build-info.js?v=29a361-chapter4-replay-overlay-fix-20260801';
...
class ArenaBattle{
  ...
  triggerSignature(fighter,type=signatureProfileFor(fighter?.id).event){
    ...
    if(!fighter.cpu)this.recordTrainingEvent(type,{fighterId:fighter.id});return true;
  }
  ...
  applyDamage(attacker,target,damage,{guardDamage=Math.max(1,damage*.2),knockback=28,color=attacker?.accent||'#ffffff',hitstop=5,stun=.24,launch=0,spike=0,knockdown=false,kind='light'}={}){
    ...
    target.hp=Math.max(0,target.hp-finalDamage);target.hurtIdentityFlash=.18;target.lastHitKind=kind;if(target.hp<=0)this.recordKoImpact(attacker,target,kind,color);registerRecoverableDamage(target,finalDamage,{strong:['heavy','airHeavy','launcher','pursuitHeavy','ultimate'].includes(kind)});target.stun=Math.max(target.stun,stun);target.inv=.045;target.flash=.12;target.guardDelay=1.1;target.kvx=aim.x*knockback;target.kvz=aim.z*knockback;
