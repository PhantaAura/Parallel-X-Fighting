...
import {clampToStage,getArenaStage,listArenaStages,outsideStageProjectileBounds,stageProfile,stageWallAvoidance} from './arena-stages.js?v=29a391-chapter4-ending-continuity-20260801';
import {drawStagePersonality,projectileHitsStageGeometry,resolveStageGeometry,stagePersonalityFor} from './stage-personality.js?v=29a391-chapter4-ending-continuity-20260801';
import {drawArenaStage} from './arena-stage-renderer.js';
import {WebGLArenaRenderer} from './webgl-renderer.js';
import {ArenaControlManager} from './arena-controls.js?v=29a391-chapter4-ending-continuity-20260801';
import {ABILITY_CATEGORY,ARENA_NORMAL_PROFILES,SPECIAL_CATEGORIES,abilityCategory,abilityTiming,arenaAttackFor,fighterFeelFor} from './arena-combat-data.js?v=29a391-chapter4-ending-continuity-20260801';
import {finalKoImpactFor,signatureMeleeKind,signatureProfileFor} from './signature-combat.js?v=29a391-chapter4-ending-continuity-20260801';
import {PLAYABLE_ROSTER_IDS,ROSTER} from '../roster.js?v=29a391-chapter4-ending-continuity-20260801';
import {COMBAT_RULES,difficultyProfile,decayHabit} from '../combat-core.js?v=29a391-chapter4-ending-continuity-20260801';
import {resolveHubWorldCollision} from '../story/hub-collision.js?v=29a391-chapter4-ending-continuity-20260801';
import {loadQolSettings} from '../qol-settings.js?v=29a391-chapter4-ending-continuity-20260801';
import {createTrainingTrialState,recordTrainingTrialEvent,resetTrainingTrial,trainingTrialView} from '../training-trials.js?v=29a391-chapter4-ending-continuity-20260801';
import {PURSUIT_TUNING,canGroundBounce,canWallSplat,dashIdentityFor,pursuitDurationFor,pursuitPromptText,pursuitTechAvailable,pursuitWindowFor} from './pursuit-combat.js?v=29a391-chapter4-ending-continuity-20260801';
import {FOCUS_RECOVERY_RULES,channelFocusRecovery,endFocusRecovery,focusRecoveryAvailability,interruptFocusRecovery,recoverableHealthView,registerRecoverableDamage,resetFocusRecovery,tickFocusRecovery} from '../focus-recovery.js?v=29a391-chapter4-ending-continuity-20260801';
import {BUILD_VERSION} from '../build-info.js?v=29a391-chapter4-ending-continuity-20260801';
import {createBattleMasterySession,finalizeBattleMastery,masteryRankLabel,recordBattleMasteryEvent,recordMasteryChallenge} from '../mastery-records.js?v=29a391-chapter4-ending-continuity-20260801';
...
class ArenaBattle{
  ...
  stepFighter(fighter,foe,command,dt){
    ...
    fighter.pursuitFollowupWindow=Math.max(0,fighter.pursuitFollowupWindow-dt);fighter.pursuitFinishWindow=Math.max(0,fighter.pursuitFinishWindow-dt);fighter.pursuitEscapeCooldown=Math.max(0,fighter.pursuitEscapeCooldown-dt);fighter.pursuitBufferFlash=Math.max(0,fighter.pursuitBufferFlash-dt);fighter.pursuitReadyFlash=Math.max(0,fighter.pursuitReadyFlash-dt);fighter.pursuitLockOn=Math.max(0,fighter.pursuitLockOn-dt);fighter.pursuitAudioTimer=Math.max(0,fighter.pursuitAudioTimer-dt);fighter.pursuitTechFlash=Math.max(0,fighter.pursuitTechFlash-dt);fighter.actionBufferTime=Math.max(0,(fighter.actionBufferTime||0)-dt);if(fighter.actionBufferTime<=0)fighter.actionBuffer='';fighter.hurtIdentityFlash=Math.max(0,(fighter.hurtIdentityFlash||0)-dt);fighter.victoryPoseTime=Math.max(0,(fighter.victoryPoseTime||0)-dt);fighter.identityPulse=Math.max(0,(fighter.identityPulse||0)-dt);fighter.signaturePulse=Math.max(0,(fighter.signaturePulse||0)-dt);fighter.signatureCooldown=Math.max(0,(fighter.signatureCooldown||0)-dt);fighter.swapAngleWindow=Math.max(0,(fighter.swapAngleWindow||0)-dt);fighter.pressureTimer=Math.max(0,(fighter.pressureTimer||0)-dt);if(fighter.pressureTimer<=0)fighter.pressureChain=0;fighter.armoredCounterWindow=Math.max(0,(fighter.armoredCounterWindow||0)-dt);fighter.landingImpact=Math.max(0,(fighter.landingImpact||0)-dt);fighter.wallSplat=Math.max(0,fighter.wallSplat-dt);fighter.groundBouncePending=Math.max(0,fighter.groundBouncePending-dt);fighter.dashArmor=Math.max(0,fighter.dashArmor-dt);fighter.juggleProtection=Math.max(0,fighter.juggleProtection-dt);
  ...
  applyDamage(attacker,target,damage,{guardDamage=Math.max(1,damage*.2),knockback=28,color=attacker?.accent||'#ffffff',hitstop=5,stun=.24,launch=0,spike=0,knockdown=false,kind='light'}={}){
    ...
    if(attacker&&!attacker.cpu){
      ...
      if(kind==='pursuitHeavy'){this.recordTrainingEvent('pursuitFinisherHit');if(attacker===this.fighters[0])recordBattleMasteryEvent(this.masterySession,'pursuitFinisher')}
