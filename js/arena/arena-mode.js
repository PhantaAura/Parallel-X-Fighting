...
import {clampToStage,getArenaStage,listArenaStages,outsideStageProjectileBounds,stageProfile,stageWallAvoidance} from './arena-stages.js?v=29a4071-chapter3-sabotage-investigation-20260802';
import {drawStagePersonality,projectileHitsStageGeometry,resolveStageGeometry,stagePersonalityFor} from './stage-personality.js?v=29a4071-chapter3-sabotage-investigation-20260802';
import {drawArenaStage} from './arena-stage-renderer.js';
import {WebGLArenaRenderer} from './webgl-renderer.js';
import {ArenaControlManager} from './arena-controls.js?v=29a4071-chapter3-sabotage-investigation-20260802';
import {ABILITY_CATEGORY,ARENA_NORMAL_PROFILES,SPECIAL_CATEGORIES,abilityCategory,abilityTiming,arenaAttackFor,fighterFeelFor} from './arena-combat-data.js?v=29a4071-chapter3-sabotage-investigation-20260802';
import {finalKoImpactFor,signatureMeleeKind,signatureProfileFor} from './signature-combat.js?v=29a4071-chapter3-sabotage-investigation-20260802';
import {PLAYABLE_ROSTER_IDS,ROSTER} from '../roster.js?v=29a4071-chapter3-sabotage-investigation-20260802';
import {COMBAT_RULES,difficultyProfile,decayHabit} from '../combat-core.js?v=29a4071-chapter3-sabotage-investigation-20260802';
import {resolveHubWorldCollision} from '../story/hub-collision.js?v=29a4071-chapter3-sabotage-investigation-20260802';
import {loadQolSettings} from '../qol-settings.js?v=29a4071-chapter3-sabotage-investigation-20260802';
import {createTrainingTrialState,recordTrainingTrialEvent,resetTrainingTrial,trainingTrialView} from '../training-trials.js?v=29a4071-chapter3-sabotage-investigation-20260802';
import {PURSUIT_TUNING,canGroundBounce,canWallSplat,dashIdentityFor,pursuitDurationFor,pursuitPromptText,pursuitTechAvailable,pursuitWindowFor} from './pursuit-combat.js?v=29a4071-chapter3-sabotage-investigation-20260802';
import {FOCUS_RECOVERY_RULES,channelFocusRecovery,endFocusRecovery,focusRecoveryAvailability,interruptFocusRecovery,recoverableHealthView,registerRecoverableDamage,resetFocusRecovery,tickFocusRecovery} from '../focus-recovery.js?v=29a4071-chapter3-sabotage-investigation-20260802';
import {BUILD_VERSION} from '../build-info.js?v=29a4071-chapter3-sabotage-investigation-20260802';
import {createBattleMasterySession,finalizeBattleMastery,masteryRankLabel,recordBattleMasteryEvent,recordMasteryChallenge} from '../mastery-records.js?v=29a4071-chapter3-sabotage-investigation-20260802';
import {applyRrvvfoBuildToFighter,buildHasPassive,currentRrvvfoBuild,enemyArchetypeShape,tuneRrvvfoAbility} from '../story/core-fun.js?v=29a4071-chapter3-sabotage-investigation-20260802';
...
function playerLoadout(fighterId){
  if(fighterId!=='rrvvfo')return PLAYER_LOADOUTS[fighterId]||PLAYER_LOADOUTS.revvfo;
  const build=currentRrvvfoBuild(),selected=new Set(build.techniques||[]),chosen=ARENA_ABILITIES.filter(ability=>selected.has(ability.id)).map(ability=>tuneRrvvfoAbility(ability,build));
  while(chosen.length<5)chosen.push({id:`buildReserve${chosen.length}`,label:'Build Reserve',icon:'◇',cost:0,cooldown:0,category:'trick',locked:true});
...
#${ID} .masteryResult{display:grid;grid-template-columns:auto 1fr;gap:8px 12px;align-items:center;margin:14px 0;padding:10px 12px;border:1px solid #ffffff38;border-radius:10px;background:#0a101cf0;text-align:left}#${ID} .masteryResult[hidden]{display:none!important}#${ID} .masteryResultRank{grid-row:1/3;display:grid;place-items:center;width:58px;height:58px;border:2px solid #fff18a;border-radius:50%;background:#17120a;color:#fff18a;font-size:34px;font-weight:1000;box-shadow:0 0 18px #ffe45f55}#${ID} .masteryResult strong{font-size:13px;letter-spacing:.06em}#${ID} .masteryResult span{color:#bfefff;font-size:10px;font-weight:900;line-height:1.35}#${ID} .masteryResultRank[data-rank="S"],#${ID} .battleRankToast[data-rank="S"] .battleRankLetter{border-color:#ffe56e;background:#443407;color:#fff4a8;box-shadow:0 0 22px #ffe56e88}#${ID} .masteryResultRank[data-rank="A"],#${ID} .battleRankToast[data-rank="A"] .battleRankLetter{border-color:#6de7ff;background:#083344;color:#c8f7ff;box-shadow:0 0 20px #6de7ff77}#${ID} .masteryResultRank[data-rank="B"],#${ID} .battleRankToast[data-rank="B"] .battleRankLetter{border-color:#75ef9a;background:#0b3820;color:#d6ffe2;box-shadow:0 0 18px #75ef9a66}#${ID} .masteryResultRank[data-rank="C"],#${ID} .battleRankToast[data-rank="C"] .battleRankLetter{border-color:#8fb9ff;background:#152a4a;color:#e4edff;box-shadow:0 0 16px #8fb9ff55}#${ID} .masteryResultRank[data-rank="D"],#${ID} .battleRankToast[data-rank="D"] .battleRankLetter{border-color:#ffb45f;background:#4a260a;color:#fff0d8;box-shadow:0 0 16px #ffb45f55}#${ID} .masteryResultRank[data-rank="E"],#${ID} .battleRankToast[data-rank="E"] .battleRankLetter{border-color:#ff7777;background:#4b1010;color:#ffe5e5;box-shadow:0 0 16px #ff777755}#${ID} .battleRankToast{position:absolute;z-index:92;left:50%;top:21%;transform:translate(-50%,-10px);display:grid;grid-template-columns:auto 1fr;gap:8px 12px;align-items:center;min-width:min(360px,82vw);padding:10px 14px;border:1px solid #ffffff52;border-radius:12px;background:#070b14f2;box-shadow:0 16px 42px #000b;opacity:1;transition:opacity .16s,transform .16s;pointer-events:none}#${ID} .battleRankToast[hidden]{display:none!important}#${ID} .battleRankToast .battleRankLetter{grid-row:1/3;display:grid;place-items:center;width:64px;height:64px;border:3px solid #8fb9ff;border-radius:50%;font-size:40px;font-weight:1000}#${ID} .battleRankToast strong{font-size:14px;letter-spacing:.1em}#${ID} .battleRankToast span{color:#d9efff;font-size:10px;font-weight:900;line-height:1.4}
...
<div class="arenaStageSelect hidden" data-arena-stage-select role="dialog" aria-modal="true" aria-label="Arena select"><div class="arenaStageCard">
  <header><small>${BUILD_VERSION}</small><h2>SELECT ARENA</h2><p>Choose a stage by its playstyle. Size, boundaries, pursuit lanes, wall pressure, and ring-out rules shape each match.</p></header>
...
<div class="badge"><strong>${BUILD_VERSION}</strong><span data-stage-name>TANGAI DOJO</span><br>REUSABLE ARENA PIPELINE • DATA-DRIVEN STAGE</div><div class="banner hidden" data-banner></div>
<div class="result hidden" data-result><small>ARENA BATTLE</small><h2 data-title>MATCH COMPLETE</h2><p data-text></p><div class="masteryResult" data-mastery-result hidden><b class="masteryResultRank" data-mastery-rank>C</b><div><strong data-mastery-title>BATTLE MASTERY</strong><span data-mastery-stats></span></div></div><div class="actions"><button class="primary" data-rematch>INSTANT REMATCH</button><button data-random-rematch>RANDOM REMATCH</button><button data-change-arena>CHANGE ARENA</button><button data-return>MAIN MENU</button></div></div>
<div class="battleRankToast" data-battle-rank-toast hidden><b class="battleRankLetter" data-battle-rank-letter>E</b><div><strong data-battle-rank-title>FIGHT RANK</strong><span data-battle-rank-detail>0/100</span></div></div>
...
<div class="arenaControlSettings hidden" data-arena-control-settings role="dialog" aria-modal="true" aria-label="Arena control settings"><div class="arenaControlCard">
  <header><div><small>${BUILD_VERSION}</small><h2>CONTROL LAYOUTS</h2><p>Desktop and mobile settings save separately from combat balance.</p></div><button data-arena-controls-close aria-label="Close">×</button></header>
...
const SPRITE_MANIFEST_IDS=new Set(['rrvvfo','revvfo','wade','bark','sage']);

async function loadSprite(renderer,id){
  if(!SPRITE_MANIFEST_IDS.has(String(id)))return null;
  try{
...
class CombatAudio{
  ...
  play(name,intensity=0,fighterId=''){
    ...
    else if(name==='signature'){this.tone(980,.045,'sine',.022,-240,.015)}
    else if(name==='finalKo'){this.tone(92,.30,'sine',.052,-24,.018);this.tone(720,.08,'square',.025,-410,.035)}
...
class ArenaBattle{
  ...
  beginBattleRank({fighterId=this.fighters[0]?.id,opponentId=this.fighters[1]?.id,stageId=this.stage?.id,mode=this.matchMode}={}){
    clearTimeout(this.rankToastTimer);const toast=this.root.querySelector('[data-battle-rank-toast]');if(toast)toast.hidden=true;
    this.masteryResult=null;this.masterySession=createBattleMasterySession({fighterId,opponentId,stageId,mode});return this.masterySession;
  }
  presentBattleRank(result,{label='FIGHT RANK',duration=2600}={}){
    if(!result)return null;const toast=this.root.querySelector('[data-battle-rank-toast]');if(!toast)return result;
    toast.dataset.rank=result.rank;toast.querySelector('[data-battle-rank-letter]').textContent=result.rank;toast.querySelector('[data-battle-rank-title]').textContent=`${label} • ${masteryRankLabel(result.rank)}`;toast.querySelector('[data-battle-rank-detail]').textContent=`${result.score}/100 • +${result.points} MASTERY PTS`;toast.hidden=false;clearTimeout(this.rankToastTimer);this.rankToastTimer=setTimeout(()=>{if(toast)toast.hidden=true},Math.max(1200,Number(duration)||2600));return result;
  }
  finalizeBattleRank({won=false,scoreFor=this.scores[0]||0,scoreAgainst=this.scores[1]||0,label='FIGHT RANK',showToast=false}={}){
    if(this.trainingMode)return null;const result=finalizeBattleMastery(this.masterySession,{won,scoreFor,scoreAgainst});this.masteryResult=result;if(showToast)this.presentBattleRank(result,{label});
    if(this.root?.classList.contains('storyEngineActive'))document.dispatchEvent(new CustomEvent('pxstoryfightrank',{detail:{rank:result.rank,score:result.score,won,label,fighterId:this.masterySession?.fighterId||this.fighters[0]?.id,opponentId:this.masterySession?.opponentId||this.fighters[1]?.id}}));
    return result;
  }

  feel(fighter){return fighterFeelFor(fighter?.id)}
  triggerSignature(fighter,type=signatureProfileFor(fighter?.id).event){
    if(!fighter)return false;const profile=signatureProfileFor(fighter.id),reward=profile.reward||{};
    fighter.signaturePulse=.62;fighter.signatureLabel=profile.label;fighter.signatureCooldown=.5;
    fighter.en=clamp(fighter.en+(reward.energy||0),0,100);fighter.momentum=clamp(fighter.momentum+(reward.momentum||0),0,100);fighter.guard=clamp(fighter.guard+(reward.guard||0),0,100);
    if(reward.cooldownReduction)fighter.cooldowns.astrylteBlast=Math.max(0,(fighter.cooldowns.astrylteBlast||0)-reward.cooldownReduction);
    this.burst(fighter.x,fighter.z,profile.color,fighter.id==='bark'?34:28,68+fighter.y);this.impactFlash(profile.color,.11);this.audio.play('signature',0,fighter.id);this.combatEvent('signature',{side:fighter===this.fighters[0]?1:2,label:profile.label,duration:820});this.notice(`${profile.label} • ${profile.detail}`,1.05);
    if(!fighter.cpu){this.recordTrainingEvent(type,{fighterId:fighter.id});if(fighter===this.fighters[0])recordBattleMasteryEvent(this.masterySession,'signature',{type})}return true;
  }
  recordKoImpact(attacker,target,kind,color){if(!attacker||!target)return;target.lastHitAttacker=attacker;target.lastHitKind=kind;target.lastHitColor=color;this.pendingKoImpact={attacker,target,kind,color}}
  stagePersonalityEnabled(){return !this.root?.classList.contains('storyEngineActive')}
  ...
  loadoutForFighter(fighter){
    const storyActive=fighter?.id==='rrvvfo'&&this.root?.classList.contains('storyEngineActive');
    const storyScripted=storyActive&&this.root?.dataset?.storyHudMode!=='combat';
    if(storyScripted)return ARENA_ABILITIES.map(ability=>ability.id==='shotsOfAgony'&&fighter?.storyShotsLocked?{...ability,label:'Unknown Technique',icon:'?',locked:true}:ability);
    if(storyActive&&Array.isArray(fighter?.storyBuildTechniques)){
      const selected=new Set(fighter.storyBuildTechniques),build=currentRrvvfoBuild();
      const chosen=ARENA_ABILITIES.filter(ability=>selected.has(ability.id)).map(ability=>tuneRrvvfoAbility(ability,build));
      if(fighter.storyShotsLocked)chosen.push({id:'unknownTechnique',label:'Unknown Technique',icon:'?',cost:0,cooldown:0,category:'trick',locked:true});
      while(chosen.length<5)chosen.push({id:`storyReserve${chosen.length}`,label:'Build Reserve',icon:'◇',cost:0,cooldown:0,category:'trick',locked:true});
      return chosen.slice(0,5);
    }
    return playerLoadout(fighter?.id);
  }

  refreshPlayerHotbar(){
    const fighter=this.fighters[0],loadout=this.loadoutForFighter(fighter);
  ...
  playerAbility(fighter,slot){return this.loadoutForFighter(fighter)[slot-1]||null}
  ...
  isNearRingEdge(fighter,margin=80){return Boolean(this.ringOutEnabled&&this.isNearStageWall(fighter,margin))}
  isRingOut(fighter){return Boolean(this.ringOutEnabled&&fighter.ringOutComplete)}
  ...
  setStage(stageId='dojo'){
    ...
    this.stage=getArenaStage(stageId);this.stageProfile=stageProfile(this.stage.id);this.ringOutEnabled=this.stageProfile?.boundary==='RING-OUT'&&this.matchMode!=='training';this.stageBoundaryPulse=0;this.crowdExcitement=0;
    const [spawn1,spawn2]=this.stage.spawnPoints;
  ...
  applyTrainingDrill(){
    ...
    else if(this.trainingDrill==='rrvvfoIdentity'){this.trainingDummy='stand';player.en=100;this.notice('RRVVFO TRIAL • OBJECT SWAP, THEN LAND A CLOSE HIT',2.4)}
    else if(this.trainingDrill==='revvfoIdentity'){this.trainingDummy='stand';this.notice('REVVFO TRIAL • CONNECT THREE CLOSE ACTIONS WITHOUT DROPPING PRESSURE',2.5)}
    else if(this.trainingDrill==='wadeIdentity'){this.trainingDummy='cpu';player.en=100;this.notice('WADE TRIAL • DASH THROUGH AN ACTIVE STRIKE',2.3)}
    else if(this.trainingDrill==='barkIdentity'){this.trainingDummy='cpu';player.en=100;this.notice('BARK TRIAL • ABSORB WITH ARMOR, THEN PUNISH',2.3)}
  ...
  recordTrainingEvent(event,detail={}){
    ...
    if(result.completed){const fighterId=this.fighters[0]?.id||'rrvvfo',grade=result.grade==='PERFECT'?'S':result.grade==='GREAT'?'A':result.grade==='GOOD'?'B':'A',saved=recordMasteryChallenge(fighterId,this.trainingDrill,{grade});this.audio.play('perfect');this.impactFlash('#fff2a3',.18);this.notice(saved.first?`${grade} RANK • MASTERY REWARD UNLOCKED`:result.grade?`${result.grade}! • TRAINING TRIAL COMPLETE`:'TRAINING TRIAL COMPLETE!',2.2)}
  }
  ...
  newRound(){const [spawn1,spawn2]=this.stage.spawnPoints;this.time=Infinity;this.phase='intro';this.phaseTime=1.25;this.hitstop=0;this.cameraShake=0;this.fighters[0].reset(spawn1.x,spawn1.z);this.fighters[1].reset(spawn2.x,spawn2.z);if(this.fighters[0].id==='rrvvfo')applyRrvvfoBuildToFighter(this.fighters[0],currentRrvvfoBuild());this.refreshPlayerHotbar();this.camera=initialCameraState(this.stage,this.fighters);this.pursuitCameraBlend=0;this.pursuitImpactHold=0;this.pursuitPromptLinger=0;this.pursuitPromptLastText='';this.particles=[];this.clearCombatObjects();this.root.classList.remove('pursuit-active');this.noticeTime=0;this.pendingKoImpact=null;this.root.classList.remove('finalKoPulse');this.notice('');const personality=stagePersonalityFor(this.stage.id);this.banner(`BATTLE ${this.round} • ${personality.label} • FIRST TO ${this.koTarget} ${this.koTarget===1?'KO':'KOs'}`);this.hud()}
  ...
  stepFighter(fighter,foe,command,dt){
    ...
    const pursuitWindowBefore=fighter.pursuitWindow,pursuitFollowupBefore=fighter.pursuitFollowupWindow,pursuitFinishBefore=fighter.pursuitFinishWindow,flowCancelBefore=fighter.flowCancelWindow;fighter.guardDelay=Math.max(0,fighter.guardDelay-dt);fighter.comboTimer=Math.max(0,fighter.comboTimer-dt);fighter.pursuitWindow=Math.max(0,fighter.pursuitWindow-dt);
    fighter.pursuitFollowupWindow=Math.max(0,fighter.pursuitFollowupWindow-dt);fighter.pursuitFinishWindow=Math.max(0,fighter.pursuitFinishWindow-dt);fighter.pursuitEscapeCooldown=Math.max(0,fighter.pursuitEscapeCooldown-dt);fighter.pursuitBufferFlash=Math.max(0,fighter.pursuitBufferFlash-dt);fighter.pursuitReadyFlash=Math.max(0,fighter.pursuitReadyFlash-dt);fighter.pursuitLockOn=Math.max(0,fighter.pursuitLockOn-dt);fighter.pursuitAudioTimer=Math.max(0,fighter.pursuitAudioTimer-dt);fighter.pursuitTechFlash=Math.max(0,fighter.pursuitTechFlash-dt);fighter.actionBufferTime=Math.max(0,(fighter.actionBufferTime||0)-dt);if(fighter.actionBufferTime<=0)fighter.actionBuffer='';fighter.hurtIdentityFlash=Math.max(0,(fighter.hurtIdentityFlash||0)-dt);fighter.victoryPoseTime=Math.max(0,(fighter.victoryPoseTime||0)-dt);fighter.identityPulse=Math.max(0,(fighter.identityPulse||0)-dt);fighter.signaturePulse=Math.max(0,(fighter.signaturePulse||0)-dt);fighter.signatureCooldown=Math.max(0,(fighter.signatureCooldown||0)-dt);fighter.swapAngleWindow=Math.max(0,(fighter.swapAngleWindow||0)-dt);fighter.flowCancelWindow=Math.max(0,(fighter.flowCancelWindow||0)-dt);fighter.pressureTimer=Math.max(0,(fighter.pressureTimer||0)-dt);if(fighter.pressureTimer<=0)fighter.pressureChain=0;fighter.armoredCounterWindow=Math.max(0,(fighter.armoredCounterWindow||0)-dt);fighter.landingImpact=Math.max(0,(fighter.landingImpact||0)-dt);fighter.wallSplat=Math.max(0,fighter.wallSplat-dt);fighter.groundBouncePending=Math.max(0,fighter.groundBouncePending-dt);fighter.dashArmor=Math.max(0,fighter.dashArmor-dt);fighter.juggleProtection=Math.max(0,fighter.juggleProtection-dt);
    if(pursuitWindowBefore>0&&fighter.pursuitWindow<=0&&!fighter.pursuitUsed){this.audio.play('pursuitEnd');this.burst(fighter.x,fighter.z,dashIdentityFor(fighter.id).trail||fighter.accent,9,12)}if((pursuitFollowupBefore>0&&fighter.pursuitFollowupWindow<=0)||(pursuitFinishBefore>0&&fighter.pursuitFinishWindow<=0))this.audio.play('pursuitEnd');if(fighter===this.fighters[0]&&flowCancelBefore>0&&fighter.flowCancelWindow<=0)this.root?.classList.remove('flowCancelReady');
  ...
  updateAttack(fighter,foe,command,dt){
    const state=fighter.attackState,def=state.def,feel=this.feel(fighter);if(command.dash&&state.hit&&fighter.flowCancelWindow>0&&fighter.en>=8){fighter.en-=8;fighter.flowCancelWindow=0;if(fighter===this.fighters[0])this.root?.classList.remove('flowCancelReady');fighter.attackState=null;fighter.dashCooldown=0;const move=normalizeMovement(command.x||state.aimX||fighter.aimX||1,command.z||state.aimZ||fighter.aimZ||0);this.startDash(fighter,foe,move);this.burst(fighter.x,fighter.z,motionIdentity(fighter.id).trail||fighter.accent,14,24);if(!fighter.cpu){this.notice('FLOW CANCEL • 8 ENERGY',.65);this.hitstop=Math.max(this.hitstop,3*STEP*this.feedback.hitstopScale);this.cameraShake=Math.max(this.cameraShake,3*this.feedback.shakeScale);this.impactFlash(motionIdentity(fighter.id).trail||fighter.accent,.08);globalThis.document?.dispatchEvent?.(new CustomEvent('pxflowcancel',{detail:{fighterId:fighter.id,engine:'arena'}}));globalThis.document?.dispatchEvent?.(new CustomEvent('pxarenafeedback',{detail:{type:'flowCancel',fighter:fighter.name}}))}return}if(command.light)state.buffer='light';if(command.heavy)state.buffer='heavy';if(command.launcher)state.buffer='launcher';if(state.elapsed>=Math.max(0,def.duration-feel.inputBuffer)){if(command.light)this.queueAction(fighter,'light');else if(command.heavy)this.queueAction(fighter,'heavy');else if(command.launcher)this.queueAction(fighter,'launcher')}
  ...
  recordEdgePressure(attacker,target,{kind,knockback}={}){
    if(!this.ringOutEnabled||target.respawnProtection>0||kind==='grab'||target.ringOutFall>0)return false;
  ...
  resolveAttackHit(attacker,target){
    const state=attacker.attackState;if(!state||state.hit)return;const def=state.def;if(state.elapsed<def.activeStart||state.elapsed>def.activeEnd)return;
    ...
    if(target.inv){if(target.id==='wade'&&target.dashTime>0&&target.signatureCooldown<=0){state.hit=true;this.triggerSignature(target,'signatureWade')}return}
  ...
  applyDamage(attacker,target,damage,{guardDamage=Math.max(1,damage*.2),knockback=28,color=attacker?.accent||'#ffffff',hitstop=5,stun=.24,launch=0,spike=0,knockdown=false,kind='light'}={}){
    ...
    if(guarded){
      ...
      const spent=(perfect?guardDamage*.18:guardDamage)*categoryScale;target.guard=Math.max(0,target.guard-spent);target.guardDelay=1.25;target.stun=perfect?.04:.115;target.kvx=aim.x*knockback*(perfect?.08:.35);target.kvz=aim.z*knockback*(perfect?.08:.35);this.hitstop=Math.max(this.hitstop,(perfect?9:3)*STEP*this.feedback.hitstopScale);this.cameraShake=Math.max(this.cameraShake,(perfect?6:2)*this.feedback.shakeScale);if(perfect&&!target.cpu){this.recordTrainingEvent('perfectParry');if(target===this.fighters[0])recordBattleMasteryEvent(this.masterySession,'perfectParry')}this.burst(target.x,target.z,perfect?'#fff4a3':'#9de7ff',perfect?18:9,58);this.audio.play(perfect?'perfect':'block');if(perfect)document.dispatchEvent(new CustomEvent('pxarenafeedback',{detail:{type:'perfectParry',target:target.name,attacker:attacker.name}}));
      if(perfect){attacker.stun=Math.max(attacker.stun,.24);const parryBonus=target.id==='rrvvfo'&&buildHasPassive(currentRrvvfoBuild(),'parrySpark')?8:0;target.en=clamp(target.en+8+parryBonus,0,100);target.momentum=clamp(target.momentum+15,0,100);if(parryBonus&&target===this.fighters[0]){this.combatEvent('abilityReady',{side:1,label:'PASSIVE • PARRY SPARK',duration:560});globalThis.document?.dispatchEvent?.(new CustomEvent('pxpassiveproc',{detail:{id:'parrySpark',fighterId:target.id}}))}this.combatEvent('perfectParry',{side:target===this.fighters[0]?1:2,label:'PERFECT PARRY'});this.pulseHud(`[data-g${target===this.fighters[0]?1:2}]`,'eventParry');this.notice('PERFECT PARRY • MOMENTUM +15');this.root.classList.remove('perfectParryPulse');void this.root.offsetWidth;this.root.classList.add('perfectParryPulse');setTimeout(()=>this.root?.classList.remove('perfectParryPulse'),360);globalThis.document?.dispatchEvent?.(new CustomEvent('pxperfectparry',{detail:{side:target.side,fighterId:target.id,cpu:target.cpu,engine:'arena'}}))}
      if(target.guard<=0){target.guard=28;target.guardBreak=.88;target.stun=.88;target.block=false;target.blockLockout=1;target.hp=Math.max(0,target.hp-2);registerRecoverableDamage(target,2,{strong:true});this.hitstop=Math.max(this.hitstop,10*STEP*this.feedback.hitstopScale);this.cameraShake=Math.max(this.cameraShake,10*this.feedback.shakeScale);if(attacker&&!attacker.cpu){this.recordTrainingEvent('guardBreak');if(attacker===this.fighters[0])recordBattleMasteryEvent(this.masterySession,'guardBreak')}this.burst(target.x,target.z,'#ffdc75',28,60);this.audio.play('guardBreak');this.combatEvent('guardBreak',{side:target===this.fighters[0]?1:2,label:'GUARD BREAK'});this.pulseHud(`[data-g${target===this.fighters[0]?1:2}]`,'eventBreak',760);document.dispatchEvent(new CustomEvent('pxarenafeedback',{detail:{type:'guardBreak',target:target.name,attacker:attacker.name}}));this.notice('GUARD BREAK!')}
      return true;
    }
    ...
    if(armored){finalDamage*=.72;stun*=.25;knockback*=.35;if(target.id==='bark')target.armoredCounterWindow=.82;this.burst(target.x,target.z,'#d7b979',16,62);this.notice(`${target.name.toUpperCase()} • STARTUP ARMOR`,.55)}
    if(target.dashArmor>0&&counterable){finalDamage*=.82;stun*=.5;knockback*=.45;if(target.id==='bark')target.armoredCounterWindow=.82;this.burst(target.x,target.z,'#d8b774',14,52);this.notice(`${target.name.toUpperCase()} • ARMORED STEP`,.5)}
    ...
    target.hp=Math.max(0,target.hp-finalDamage);if(attacker===this.fighters[0])recordBattleMasteryEvent(this.masterySession,'hit',{damage:finalDamage,combo:(attacker.comboTimer>0?attacker.comboHits+1:1),kind});if(target===this.fighters[0])recordBattleMasteryEvent(this.masterySession,'damageTaken',{damage:finalDamage,kind});target.hurtIdentityFlash=.18;target.lastHitKind=kind;if(target.hp<=0)this.recordKoImpact(attacker,target,kind,color);registerRecoverableDamage(target,finalDamage,{strong:['heavy','airHeavy','launcher','pursuitHeavy','ultimate'].includes(kind)});target.stun=Math.max(target.stun,stun);target.inv=.045;target.flash=.12;target.guardDelay=1.1;target.kvx=aim.x*knockback;target.kvz=aim.z*knockback;
    ...
    if(attacker){
      if(!attacker.cpu&&['light1','light2','light3','heavy','airLight','airHeavy','launcher'].includes(kind)){attacker.flowCancelWindow=.30;if(attacker===this.fighters[0]){this.root?.classList.add('flowCancelReady');this.audio.play('pursuitBuffer')}};
      ...
      const closeDistance=Math.hypot(attacker.x-target.x,attacker.z-target.z);
      if(attacker.id==='rrvvfo'&&attacker.swapAngleWindow>0&&signatureMeleeKind(kind)){attacker.swapAngleWindow=0;this.triggerSignature(attacker,'signatureRrvvfo')}
      if(attacker.id==='revvfo'&&signatureMeleeKind(kind)&&closeDistance<160){attacker.pressureChain=attacker.pressureTimer>0?attacker.pressureChain+1:1;attacker.pressureTimer=.86;if(attacker.pressureChain>=3){attacker.pressureChain=0;attacker.pressureTimer=0;this.triggerSignature(attacker,'signatureRevvfo')}}
      if(attacker.id==='bark'&&attacker.armoredCounterWindow>0&&signatureMeleeKind(kind)){attacker.armoredCounterWindow=0;this.triggerSignature(attacker,'signatureBark')}
    }
    ...
    if(attacker&&!attacker.cpu){
      ...
      if(kind==='pursuitHeavy'){this.recordTrainingEvent('pursuitFinisherHit');if(attacker===this.fighters[0]){recordBattleMasteryEvent(this.masterySession,'pursuitFinisher');globalThis.document?.dispatchEvent?.(new CustomEvent('pxarenafeedback',{detail:{type:'pursuitFinisher',target:target.name,attacker:attacker.name}}));if(attacker.id==='rrvvfo'&&buildHasPassive(currentRrvvfoBuild(),'pursuitBattery')){attacker.en=clamp(attacker.en+7,0,100);this.combatEvent('abilityReady',{side:1,label:'PASSIVE • PURSUIT BATTERY',duration:560});globalThis.document?.dispatchEvent?.(new CustomEvent('pxpassiveproc',{detail:{id:'pursuitBattery',fighterId:attacker.id}}))}}}
  ...
  updateSpecials(dt){
    ...
    for(const projectile of this.projectiles){
      ...
      const geometryHit=this.stagePersonalityEnabled()&&projectile.life>0?projectileHitsStageGeometry(this.stage,projectile):null;if(geometryHit){projectile.life=0;this.burst(projectile.x,projectile.z,geometryHit.accent||'#ffffff',16,55);this.audio.play('block')}
  ...
  finishRound(){if(this.phase!=='play')return;const[a,b]=this.fighters,winner=b.hp>a.hp?1:a.hp>b.hp?0:(Math.random()<.5?0:1),winnerFighter=this.fighters[winner],loser=this.fighters[1-winner],finalKo=this.scores[winner]+1>=this.koTarget,impact=finalKoImpactFor(winnerFighter.id,this.pendingKoImpact?.kind||loser.lastHitKind||'light');this.lastLoser=1-winner;this.scores[winner]++;if(finalKo&&winner===0)recordBattleMasteryEvent(this.masterySession,'finalKo');winnerFighter.victoryPoseTime=finalKo?3.05:2.4;loser.victoryPoseTime=0;this.phase='over';this.phaseTime=finalKo?impact.hold:.72;for(const fighter of this.fighters){this.clearPursuitState(fighter);endFocusRecovery(fighter,{applyRecovery:false})}this.root.classList.remove('pursuit-active');if(finalKo){this.hitstop=Math.max(this.hitstop,impact.hitstopFrames*STEP*this.feedback.hitstopScale);this.cameraShake=Math.max(this.cameraShake,impact.shake*this.feedback.shakeScale);this.impactFlash(impact.color,.28);this.burst(loser.x,loser.z,impact.color,impact.burst,72+loser.y);this.root.classList.remove('finalKoPulse');void this.root.offsetWidth;this.root.classList.add('finalKoPulse');this.banner(`${impact.label} • ${this.scores[0]}–${this.scores[1]}`);this.audio.play('finalKo',0,winnerFighter.id)}else{this.banner(`K.O. • ${this.scores[0]}–${this.scores[1]}`);this.audio.play('ko')}this.hud()}
  showResult(){this.phase='result';this.hideBanner();const winner=this.scores[0]>this.scores[1]?0:1,result=this.root.querySelector('[data-result]'),random=result.querySelector('[data-random-rematch]');if(random)random.classList.toggle('hidden',!this.allowRandomRematch);result.querySelector('[data-title]').textContent=`${this.fighters[winner].name.toUpperCase()} WINS`;result.querySelector('[data-text]').textContent=`First to ${this.koTarget} ${this.koTarget===1?'KO':'KOs'} complete. Final score ${this.scores[0]}–${this.scores[1]}. KOs flowed continuously without resetting the winner.`;if(!this.trainingMode){this.masteryResult=this.finalizeBattleRank({won:winner===0,scoreFor:this.scores[0],scoreAgainst:this.scores[1]});const mastery=result.querySelector('[data-mastery-result]');if(mastery&&this.masteryResult){mastery.hidden=false;const rankNode=mastery.querySelector('[data-mastery-rank]');rankNode.textContent=this.masteryResult.rank;rankNode.dataset.rank=this.masteryResult.rank;mastery.querySelector('[data-mastery-title]').textContent=`FIGHT RANK • ${this.masteryResult.score}/100 • +${this.masteryResult.points} PTS`;const stats=this.masteryResult.stats;mastery.querySelector('[data-mastery-stats]').textContent=`Best combo ${stats.bestCombo} • Parries ${stats.perfectParries} • Guard breaks ${stats.guardBreaks} • Pursuit finishers ${stats.pursuitFinishers} • Signature moments ${stats.signatures}`}}result.classList.remove('hidden')}
  ...
  hud(){
    ...
    const lensCosts=this.lensCosts(),loadout=this.loadoutForFighter(player);this.root.querySelectorAll('[data-arena-slot]').forEach((button,index)=>{
  ...
  drawCombatEffects(r){
    ...
    for(const fighter of this.fighters){
      ...
      if(fighter.signaturePulse>0){const profile=signatureProfileFor(fighter.id),alpha=clamp(fighter.signaturePulse/.62,0,1),pulse=1+Math.sin(performance.now()/42)*.1;r.disc({x:fighter.x,y:8,z:fighter.z,rx:68*pulse,rz:44*pulse,color:profile.color,alpha:.28*alpha});r.billboard({x:fighter.x,y:78+fighter.y,z:fighter.z,size:156*pulse,color:profile.color,alpha:.18*alpha})}
      if(fighter.pursuitTime>0){
  ...
  drawFallback2D(context,fighter,rect){
    const roster=ROSTER[fighter.id]||{},accent=fighter.accent||roster.a||'#6b7280',body=fighter.id==='rrvvfo'?'#be2026':(roster.c||accent),hair=fighter.id==='rrvvfo'?'#9a6a45':(roster.h||accent),skin=fighter.skinColor||'#9a5b3d';
    ...
    context.fillStyle=body;context.fillRect(cx-bodyW/2,rect.y+70*scale,bodyW,bodyH);
    context.fillStyle=skin;context.beginPath();context.arc(cx,rect.y+50*scale,head,0,Math.PI*2);context.fill();
    context.fillStyle=hair;context.fillRect(cx-24*shape.head*scale,rect.y+21*scale,48*shape.head*scale,24*scale);
  ...
  drawFallback(r,fighter){const foe=this.fighters.find(candidate=>candidate!==fighter),angle=foe?Math.atan2(foe.x-fighter.x,foe.z-fighter.z):0,roster=ROSTER[fighter.id]||{},accent=fighter.accent||roster.a||'#6b7280',skin=fighter.skinColor||'#9a5b3d',body=fighter.id==='rrvvfo'?'#be2026':(roster.c||accent),hair=fighter.id==='rrvvfo'?'#9a6a45':(roster.h||accent),shape=enemyArchetypeShape(fighter.aiArchetype);r.box({x:fighter.x,y:60+fighter.y,z:fighter.z,sx:48*shape.width,sy:82*shape.height,sz:30*shape.width,rotationY:angle,color:body});r.box({x:fighter.x,y:122+fighter.y,z:fighter.z,sx:39*shape.head,sy:39*shape.head,sz:36*shape.head,rotationY:angle,color:skin});r.box({x:fighter.x,y:148+fighter.y,z:fighter.z-2,sx:46*shape.head,sy:24,sz:43*shape.head,rotationY:angle,color:hair});const arm=10+Math.sin(fighter.animationClock/90)*7;r.box({x:fighter.x+Math.cos(angle)*arm*shape.width,y:74+fighter.y,z:fighter.z-Math.sin(angle)*arm*shape.width,sx:15,sy:62*shape.height,sz:15,rotationY:angle,color:skin})}
}
