...
function render(){
  ...
  world.fighters.forEach((fighter,index)=>{const labels=[];if(fighter.id==='rrvvfo'){if(fighter.agonyActiveVolley)labels.push('◉ SHOTS VOLLEY ACTIVE');else if(fighter.agonyCooldown>0)labels.push(`◷ SHOTS ${Math.ceil(fighter.agonyCooldown/60)}s`);if(fighter.lens>0)labels.push(`◉ LENS ${Math.ceil(fighter.lens/60)}s`);else if(fighter.lensCooldown>0)labels.push(`◷ LENS ${Math.ceil(fighter.lensCooldown/60)}s`);if(fighter.dashCd>0)labels.push(`◷ SWAP ${Math.ceil(fighter.dashCd/60)}s`);labels.push(fighter.hp>50?'⚠ LENS COSTS 50 HP':'⚠ LENS STOPS AT 1 HP')}if(fighter.ultCd>0)labels.push(`◷ ULT ${Math.ceil(fighter.ultCd/60)}s`);cooldownHud[index].textContent=labels.join(' • ')});
  ...
  const [a,b]=world.fighters;U.p1h.style.width=a.hp+'%';U.p2h.style.width=b.hp+'%';U.p1e.style.width=a.en+'%';U.p2e.style.width=b.en+'%';U.p1g.style.width=a.guard+'%';U.p2g.style.width=b.guard+'%';U.p1hpText.textContent=Math.ceil(a.hp);U.p2hpText.textContent=Math.ceil(b.hp);U.p1enText.textContent=Math.floor(a.en);U.p2enText.textContent=Math.floor(b.en);U.p1guardText.textContent=Math.ceil(a.guard);U.p2guardText.textContent=Math.ceil(b.guard);U.p1d.textContent=`${a.en>=90&&!a.ultCd?'◆ ULT READY':'◇ ULT CHARGING'} • ${a.breakerUsed?'× BREAKER SPENT':'◇ BREAKER READY'}`;U.p2d.textContent=`${b.en>=90&&!b.ultCd?'◆ ULT READY':'◇ ULT CHARGING'} • ${b.breakerUsed?'× BREAKER SPENT':'◇ BREAKER READY'}`;const status=fighter=>[fighter.armor?'ARMOR':null,fighter.aura?'AURA':null,fighter.freeze?'FROZEN':null,fighter.stun?'STUN':null,fighter.knockdown?'KNOCKDOWN':null,fighter.guardBreakStun?'GUARD BROKEN':null].filter(Boolean).join(' • ');U.p1status.textContent=status(a);U.p2status.textContent=status(b);U.timer.textContent=Math.ceil(time);
}
