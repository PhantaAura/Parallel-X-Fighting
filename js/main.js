"use strict";
import {FIGHTER_META,FIGHTER_STATUS,PLAYABLE_ROSTER_IDS,ROSTER,ROSTER_IDS,isMirrorMatch} from './roster.js?v=29a40-core-fun-overhaul-20260801';
import {STAGES,drawStage} from './stages.js';
import {CUSTOM_CONTROLLER_ACTIONS} from './input.js?v=29a40-core-fun-overhaul-20260801';
import {sharedInput as input} from './input-runtime.js?v=29a40-core-fun-overhaul-20260801';
import {decideCPU} from './ai.js';
import {TimerRegistry,clamp,resetCombo} from './combat.js';
import {EffectSystem} from './effects.js';
import {Fighter} from './fighter.js?v=29a40-core-fun-overhaul-20260801';
...
import {TouchControls} from './touch-controls.js?v=29a40-core-fun-overhaul-20260801';
import {TouchSettingsPanel,createDefaultTouchSettings} from './touch-layout-editor.js?v=29a40-core-fun-overhaul-20260801';
import {FighterVisuals,availableRrvvfoAppearances,isDeveloperSpriteBuild,loadRrvvfoVisualSettings,normalizeRrvvfoAppearance,shouldShowRrvvfoLoadFailure} from './fighter-visuals.js?v=29a40-core-fun-overhaul-20260801';
import {SpriteDebugViewer} from './sprite-debug-viewer.js';
import {ControllerManager} from './controller-manager.js?v=29a40-core-fun-overhaul-20260801';
import {BUILD_VERSION} from './build-info.js?v=29a40-core-fun-overhaul-20260801';
import {initializeStoryPolish} from './story/story-polish.js?v=29a40-core-fun-overhaul-20260801';
import {initializeStoryCharm} from './story/story-charm.js?v=29a40-core-fun-overhaul-20260801';
import {initializeMobileStoryUi} from './story/mobile-story-ui.js?v=29a40-core-fun-overhaul-20260801';
import {ConfirmationDialog} from './confirmation-dialog.js';
import {MainMenu} from './main-menu.js?v=29a40-core-fun-overhaul-20260801';
import {MatchStatistics} from './match-statistics.js';
import {PauseMenu,simulationCanAdvance} from './pause-menu.js?v=29a40-core-fun-overhaul-20260801';
import {ResultsScreen} from './results-screen.js';
import {loadQolSettings,saveQolSettings} from './qol-settings.js?v=29a40-core-fun-overhaul-20260801';
import {NotificationSystem} from './notification-system.js';
import {adaptiveMoveList,renderAdaptiveMoveList} from './move-list.js';
import {SettingsPanel} from './settings-panel.js?v=29a40-core-fun-overhaul-20260801';
import {importSaveText,resetSaveGroup,stringifySave} from './save-manager.js?v=29a40-core-fun-overhaul-20260801';
...
import {cooldownText,fighterHudModel} from './hud-model.js?v=29a40-core-fun-overhaul-20260801';
import {AbilityHotbar} from './ability-hotbar.js?v=29a40-core-fun-overhaul-20260801';
import {loadAbilityHotbarSettings,saveAbilityHotbarSettings} from './ability-hotbar-data.js?v=29a40-core-fun-overhaul-20260801';
import {ResponsiveGameLayout} from './responsive-game-layout.js?v=29a40-core-fun-overhaul-20260801';
import {OrientationManager,loadMobilePresentationSettings,saveMobilePresentationSettings} from './orientation-manager.js?v=29a40-core-fun-overhaul-20260801';
import {FullscreenManager} from './fullscreen-manager.js?v=29a40-core-fun-overhaul-20260801';
import {loadLostYearProgress,modeUnlockedForProgress,modeUnlockRequirement} from './story/lost-year-data.js?v=29a40-core-fun-overhaul-20260801';
...
function handleSettingsAction(action){
  ...
  if(action==='resetHubCamera'){import('./story/hub-camera.js?v=29a40-core-fun-overhaul-20260801').then(({resetAllHubCameras})=>{resetAllHubCameras();notifications.push('HUB CAMERA RESET',{important:true,key:'camera-reset'})});return}
...
function showExtra(section){const content=$('extrasContent');if(section==='manual'){import('./story/combat-manual.js?v=29a40-core-fun-overhaul-20260801').then(({openCombatManual})=>{openCombatManual({grantPublic:true,onClose:()=>{$('extrasPanel').classList.remove('hidden')}});$('extrasPanel').classList.add('hidden')});return}if(section==='moves'){openAdaptiveMoveList(p1id,1);return}if(section==='profiles')content.innerHTML=Object.entries(ROSTER).map(([id,fighter])=>`<p><strong>${fighter.n}</strong> — ${FIGHTER_META[id].style} <small>• ${FIGHTER_STATUS[id]?.label||'PROTOTYPE'}</small></p>`).join('');else if(section==='stages')content.innerHTML=Object.entries(STAGES).map(([id,data])=>`<p><strong>${data.n}</strong> — ${STAGE_DETAILS[id].description}</p>`).join('');else if(section==='controls'){openControlsPanel();return}else if(section==='mastery'){import('./mastery-records.js?v=29a40-core-fun-overhaul-20260801').then(({loadMasteryRecords,renderMasteryRecords})=>{content.innerHTML=renderMasteryRecords(loadMasteryRecords(resilientStorage))});return}else if(section==='builds'||section==='adventure'){import('./story/core-fun.js?v=29a40-core-fun-overhaul-20260801').then(({renderCoreFunExtras,saveRrvvfoBuild})=>{content.innerHTML=renderCoreFunExtras({storage:resilientStorage});content.querySelectorAll('[data-core-build]').forEach(button=>button.addEventListener('click',()=>{saveRrvvfoBuild(button.dataset.coreBuild,resilientStorage);content.innerHTML=renderCoreFunExtras({storage:resilientStorage});showExtra('builds')}))});return}else if(section==='credits')content.innerHTML='<strong>Parallels X: Clash of Souls</strong><p>Original characters and game direction by the Parallels X creator. Browser prototype engineering developed collaboratively with Codex.</p>';else content.innerHTML=`<strong>${BUILD_VERSION}</strong><p>Living Tournament Hub • Chapters 1–4 • Story RPG stats • Side quests • Tournament intermissions • Kinetic combat</p>`}
